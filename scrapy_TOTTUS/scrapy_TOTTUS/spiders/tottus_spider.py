...
class TottusSpider(CrawlSpider):
    ...
    # start_urls = ['https://www.tottus.com.pe/{}'.format(busqueda)]
    ...
    def parse_filter(self, response):
        ...
        titulo = response.xpath("//div[@class='column-right']/h1/text()").get()
        #link = response.xpath("//div[@class='ui-pdp-container__row ui-pdp-container__row--main-actions']/form/div/input[2]/@value").get()
        
        precio = response.xpath("//div[@class='ProductPrice']/span/span[1]/text()").get()
        if precio.split(' ')[0]=='s/':
            precio = float(precio.split(' ')[1])
        else:
            precio = nan
